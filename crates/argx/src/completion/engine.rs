...
mod tests {
    ...
    fn used_scalar_options_are_suppressed_but_repeatable_options_remain() {
        let candidates = values("tool get --output out --field id --");
        assert!(!candidates.contains(&"--output".into()));
    ...
    fn arbitrary_values_do_not_offer_structural_candidates() {
        assert!(values("tool get --output ").is_empty());
        assert!(values("tool get --output=pa").is_empty());
        assert!(values("tool get -opa").is_empty());
    }
