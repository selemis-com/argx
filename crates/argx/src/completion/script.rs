...
use super::{
    ...
    PROTOCOL_LINE_ENV, PROTOCOL_VERSION, PROTOCOL_WORDS_ENV, ScriptError, Shell,
};
...
_argx_complete_{command}() {{
    local __argx_out __argx_line __argx_value __argx_prefix=
    __argx_out="$({PROTOCOL_ENV}={PROTOCOL_VERSION} \
        {PROTOCOL_LINE_ENV}="${{COMP_LINE:0:$COMP_POINT}}" \
        {PROTOCOL_BASH_WORD_ENV}="${{COMP_WORDS[COMP_CWORD]}}" \
        {PROTOCOL_BASH_WORDBREAKS_ENV}="$COMP_WORDBREAKS" \
        command '{command}' {PROTOCOL_COMMAND} 2>/dev/null)" || return 0

    COMPREPLY=()
    while IFS= read -r __argx_line; do
        case "$__argx_line" in
            $'\001prefix\t'*) __argx_prefix="${{__argx_line#*$'\t'}}" ;;
            '') ;;
            *)
    ...
    if [[ -n $__argx_prefix ]]; then
        local __argx_i
        for __argx_i in "${{!COMPREPLY[@]}}"; do
            [[ ${{COMPREPLY[__argx_i]}} == "$__argx_prefix"* ]] &&
                COMPREPLY[__argx_i]="${{COMPREPLY[__argx_i]#"$__argx_prefix"}}"
        done
    fi
}}
...
mod tests {
    ...
    fn scripts_use_the_private_protocol_and_register_the_requested_shell() {
        ...
        assert!(bash.contains("ARGX_COMPLETE_BASH_WORD="));
        assert!(bash.contains("ARGX_COMPLETE_BASH_WORDBREAKS="));
        assert!(bash.contains("$'\\001prefix\\t'*"));
