...
mod tests {
    ...
    #[derive(Debug, argx::Config)]
    struct DelimitedEnvironmentConfig {
        #[argx(env = "ARGX_DELIMITED_VALUES", delimited)]
        values: Vec<String>,
    }

    #[test]
    fn delimited_environment_values_use_the_same_comma_syntax_as_argv() {
        let path =
            temp_file("environment-delimited", "env", "ARGX_DELIMITED_VALUES=alpha,beta,gamma\n");

        let config = DelimitedEnvironmentConfig::loader()
            .layer(Dotenv::new(&path))
            .resolve()
            .expect("resolve comma-delimited environment value");

        assert_eq!(config.values, ["alpha", "beta", "gamma"]);
        let _ = fs::remove_file(path);
    }
